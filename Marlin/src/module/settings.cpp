...
#include "muyu.h"
  ...
  #endif
#define SHOW_PARAM_INDEX            0
#define SHOW_HERE_INDEX(x)          SERIAL_IMPL.print(x);SERIAL_IMPL.println(eeprom_index)
#endif
...
typedef struct SettingsDataStruct {
  ...
  uint16_t  crc;                                        // Data Checksum
    ...
    uint8_t dual_x_carriage_mode;//DUAL_X_CARRIAGE
    Touch_t para_calibration;
    float fifth_e_factor;
    int16_t babystep_accum;
  #endif
  ...
  #else //让下面的参数保持位置固定,注意:当HOTENDS>2时,这里位置会变化,造成异常
    xyz_pos_t hotend_offset;
  #endif
  #if HOTENDS > 2
  #error settings parameter position changed for hotends > 2
  #endif
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    #if TRONXY_UI
      #if SHOW_PARAM_INDEX
      SHOW_HERE_INDEX("my_lang-addr:");
      #endif
      EEPROM_WRITE(my_lang);
      EEPROM_WRITE(buzzer_switch);
      #if DISABLED(DUAL_X_CARRIAGE)
      const uint8_t dual_x_carriage_mode = 0x01;
      #endif
      EEPROM_WRITE(dual_x_carriage_mode);
      EEPROM_WRITE(yWindow::para_calibration);
      EEPROM_WRITE(fifth_e_factor);
      #if ENABLED(BABYSTEPPING)
      EEPROM_WRITE(babystep.accum);
      #else
      const int16_t accum = 0xFFFF;
      EEPROM_WRITE(accum);
      #endif
    #endif
    ...
    {
      #if SHOW_PARAM_INDEX
      SHOW_HERE_INDEX("probe-offset:");
      #endif
      _FIELD_TEST(probe_offset);
      #if HAS_BED_PROBE
        const xyz_pos_t &zpo = TERN(HAS_TRONXY_UI,Probe_Offset,probe.offset);
      #else
    ...
    if (!eeprom_error) {
      ...
      TERN(HAS_TRONXY_PROJ,EEPROM_WRITE(tronxy_version),EEPROM_WRITE(version));
  ...
  void MarlinSettings::first_load() 
  {
        ...
        case DXC_FULL_CONTROL_MODE:QUEUE_CMD("M605 S0");break;
        case DXC_AUTO_PARK_MODE   :QUEUE_CMD("M605 S1");break;
        case DXC_DUPLICATION_MODE :QUEUE_CMD("M605 S2 X150 R0");break;
        case DXC_MIRRORED_MODE    :
          QUEUE_CMD("M605 S2 X150 R0");
          QUEUE_CMD("M605 S3");
          break;
        }
        #endif
  ...
  bool MarlinSettings::_load() {
    ...
    auto mycmp = [&]() {
      int i = 0;
      for(; i < 4; i++)if(tronxy_version[i] != stored_ver[i])break;
      return i < 4;
    };
    if(mycmp()) {
      DEBUG_ECHO_MSG("EEPROM version mismatch (EEPROM=",
      (int)stored_ver[0], ".",
      (int)stored_ver[1], ".",
      (int)stored_ver[2], ".",
      (int)stored_ver[3]);
      DEBUG_ECHO_MSG(" Marlin=" ,
      (int)tronxy_version[0], ".",
      (int)tronxy_version[1], ".",
      (int)tronxy_version[2], ".",
      (int)tronxy_version[3],")");
      eeprom_error = true;
    }
      ...
      working_crc = 0;  // Init to 0. Accumulated by EEPROM_READ
        ...
        #endif
        EEPROM_READ(my_lang);
        EEPROM_READ(buzzer_switch);
        #if DISABLED(DUAL_X_CARRIAGE)
        uint8_t dual_x_carriage_mode;
        #endif
        EEPROM_READ(dual_x_carriage_mode);
        EEPROM_READ(yWindow::para_calibration);
        EEPROM_READ(fifth_e_factor);
        #if ENABLED(BABYSTEPPING)
        EEPROM_READ(babystep.accum);
        #else
        int16_t accum;
        EEPROM_READ(accum);
        #endif
      ...
      //
      {
        ...
        #else
          volatile xyz_pos_t hotend_offset;
      ...
      //
      {
        ...
        #if ENABLED(AUTO_BED_LEVELING_BILINEAR)
        set_bed_leveling_enabled(planner_leveling_active);
        #endif
      }
      ...
      if (eeprom_error) {
        ...
        DEBUG_ECHO_START();
        DEBUG_ECHOLNPGM("V",tronxy_version[0],".",tronxy_version[1],".",tronxy_version[2],".",tronxy_version[3]);
        #ifdef EEPROM_AT24CXX 
        DEBUG_ECHOLNPGM(" stored settings retrieved (", eeprom_index - (EEPROM_OFFSET), " bytes)");
        #else
...
void MarlinSettings::reset() {
  ...
  //
    ...
    static_assert(COUNT(dpo) == NUM_AXES, "NOZZLE_TO_PROBE_OFFSET must contain offsets for each linear axis X, Y, Z....");
      ...
      LOOP_NUM_AXES(a) TERN(HAS_TRONXY_UI,Probe_Offset[a],probe.offset[a]) = dpo[a];
    #else
      TERN(HAS_TRONXY_UI,Probe_Offset,probe.offset).set(NUM_AXIS_LIST(0, 0, dpo[Z_AXIS], 0, 0, 0, 0, 0, 0));
    #endif
  #endif
