...
static bool ensure_safe_temperature(const bool wait=true, const PauseMode mode=PAUSE_MODE_SAME) {
  ...
  DEBUG_ECHOLNPGM("... wait:", wait, " mode:", mode);
    ...
    yevt.addEvt(TASK_SEQUENCE,popupWaitNozzleHeat);
    #endif
  #else
  ...
  #if TRONXY_UI
  if(messageSt.currentIndex == POPUP_WAITNOZZLE_HEAT) closePopupMessage();
...
bool pause_print(const_float_t retract, const xyz_pos_t &park_point, const bool show_lcd/*=false*/, const_float_t unload_length/*=0*/ DXC_ARGS) {
  ...
  print_job_timer.pause();
    ...
    #endif
    #if ENABLED(SPINDLE_LASER_USE_PWM)
    recovery.info.unitPower = cutter.unitPower;
    cutter.unitPower = 0;
    cutter.set_ocr(cutter.unitPower);
    #endif
...
void resume_print(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/, const celsius_t targetTemp/*=0*/ DXC_ARGS) {
  ...
  #if TRONXY_UI
    #if HAS_HOTEND
    if(recovery.info.target_temperature[active_extruder] >= thermalManager.extrude_min_temp && !ensure_safe_temperature(DISABLED(BELTPRINTER)))return;
    #endif
  #else
  ...
  if (!axes_should_home()) {
    // Move XY back to saved position
      current_position.x = recovery.info.current_position.x;
      current_position.y = recovery.info.current_position.y;
      line_to_current_position(NOZZLE_PARK_XY_FEEDRATE);
      planner.synchronize();

    // Move Z back to saved position
      current_position.z = recovery.info.current_position.z;
  ...
  #if TRONXY_UI
    planner.set_e_position_mm(current_position.e = recovery.info.current_position.e);
    queue.clear();
    #if ENABLED(SDSUPPORT)
    card.setIndex(recovery.info.sdpos);
    #endif
    #if ENABLED(SPINDLE_LASER_USE_PWM)
    cutter.unitPower = recovery.info.unitPower;
    cutter.set_ocr(cutter.unitPower);
    #endif
    my_print_status = PRINT_RUNNING;
    if(enabled_taoli)WRITE(pin_air,TAOLI_AIR_OPEN);
    feedrate_mm_s = recovery.info.feedrate;
  #else
