...
#define CONST_CMD(x) gcode.process_subcommands_now(F(x))
#define QUEUE_CMD(x) queue.enqueue_one(F(x))
#define JOIN_CMD(x)  queue.enqueue_one(x)
extern const uint8_t tronxy_version[4];
...
void screenKillError(FSTR_P const errorInfo1, FSTR_P const errorInfo2);
...
extern uint8_t my_lang,buzzer_switch;
...
extern FSTR_P screen_error1, screen_error2;
extern xyz_pos_t Probe_Offset;
extern const char* firmware_name;
int FSTR_P_len(FSTR_P t);
#define TAOLI_AIR_OPEN                HIGH
...
constexpr int INDEX_LANG = 108;
constexpr int INDEX_BUZZER_SWITCH = INDEX_LANG + 1;
constexpr int INDEX_DUAL_X_MODE = INDEX_BUZZER_SWITCH + 1;
constexpr int INDEX_CALIBRATION = INDEX_DUAL_X_MODE + 1;
constexpr int INDEX_FIFTH_EFACTOR = INDEX_CALIBRATION + sizeof(Touch_t);
constexpr int INDEX_BABYSTEP_ACCUM = INDEX_FIFTH_EFACTOR + 4;
constexpr int INDEX_PROBE_OFFSET = INDEX_BABYSTEP_ACCUM + 2;
constexpr int INDEX_HOTEND_OFFSET = INDEX_PROBE_OFFSET + sizeof(Probe_Offset);
constexpr int INDEX_GRID_SPACING = INDEX_HOTEND_OFFSET + sizeof(xyz_pos_t) + 2;
